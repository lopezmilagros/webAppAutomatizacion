...
const Placeholder = () => (
    <div className="flex flex-col items-center justify-center h-full text-slate-400 group-hover:text-slate-500 transition-colors duration-300">
        ...
        <span className="font-sans font-medium text-base">Subir foto</span>
    </div>
);
...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, dragConstraintsRef, onShake, onDownload, isMobile }) => {
    ...
    const cardInnerContent = (
        <>
            <div className="w-full bg-slate-50 shadow-inner flex-grow relative overflow-hidden group">
                        ...
                        <div className={cn(
                            ...
                            {onDownload && (
                                <button
                                    ...
                                    className="p-2 bg-slate-700/75 rounded-full text-white hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-600 transition-colors"
                                    aria-label={`Descargar imagen para ${caption}`}
                                >
                            ...
                            {isMobile && onShake && (
                                <button
                                    ...
                                    className="p-2 bg-slate-700/75 rounded-full text-white hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-600 transition-colors"
                                    aria-label={`Volver a generar imagen para ${caption}`}
                                >
            ...
            <div className="absolute bottom-4 left-4 right-4 text-center px-2">
                <p className={cn(
                    ...
                    status === 'done' && imageUrl ? 'text-slate-800' : 'text-slate-700'
                )}>
                    {caption}
                </p>
            </div>
    ...
    if (isMobile) {
        return (
            <div className="bg-white !p-4 !pb-16 flex flex-col items-center justify-start aspect-[3/4] w-80 max-w-full rounded-lg shadow-md border border-slate-100 relative">
                {cardInnerContent}
            </div>
        );
    }
    ...
    return (
        <DraggableCardContainer>
            <DraggableCardBody
                className="bg-white !p-4 !pb-16 flex flex-col items-center justify-start aspect-[3/4] w-80 max-w-full border border-slate-100 rounded-lg"
